...
class Product_model extends App_model {
    ...
    /*public function getProducts() {
        ...
        IFNULL(GROUP_CONCAT(p_a.id_age),"") as age, IFNULL(GROUP_CONCAT(p_p.id_pathologies),"") as pathology, IFNULL(GROUP_CONCAT(p_c.idCategory),"") as category';
        $this->db->select($query)
        ->from('products p')
        ->join('product_category p_c', 'p_c.idProduct= p.id', 'left')
        ->join('product_age p_a', 'p_a.idProduct= p.id', 'left')
        ->join('product_pathologies p_p', 'p_p.idProduct= p.id', 'left')
    ...
    public function getProducts($gender, $age, $list_categories, $list_pathologies, $order, $keywords) {      
        $User = $this->session->userdata('user_logged');
        $field='salePublic';
        if ($User !=null and $User['type']==2) {
            $field='saleRetailer';
        }
        $columns = array('p.name'=> 2,'p.description'=> 0.2,'p.summary'=> 0.2,'p.brand'=> 0.01,'p.ingredients'=> 0.2,'p.productCod'=> 1.5,'p.keywords'=> 0.7,'p.others'=> 0.2);
        ...
        if ($gender!=NULL && $gender!=0) {
            $this->db->join('product_gender p_g', 'p_g.idProduct= p.id')->where('p_g.id_gender', $gender);
        }
        if ($age!=NULL && $age!=0) {
            $this->db->join('product_age p_a', 'p_a.idProduct= p.id')->where('p_a.id_age', $age);
        }
        if ($list_categories!=NULL) {
            $this->db->join('product_category p_c', 'p.id = p_c.idProduct');
        ...
        if ($list_pathologies!=NULL) {
            $this->db->join('product_pathologies p_p', 'p_p.idProduct= p.id');
            $this->db->where_in('p_p.idPathology', $list_pathologies);
            /*$this->db->or_group_start();
            var_dump($list_pathologies);
            exit();
            foreach ($list_pathologies as $pathology) {
                $this->db->where('(select COUNT(*) as count from  product_pathologies where `id` = `product_pathologies`.`idProduct` AND product_pathologies.idPathology='.intval($pathology) .')= 0 ' , NULL, FALSE );
                //$this->db->where('p_p.idPathology ', intval($pathology));
            }
            $this->db->group_end();*/
